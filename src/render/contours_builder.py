...
def build_seed_polylines(
    ...
    if num_workers > 1 and len(levels) > 1:
        ...
        def process_level(li_level: tuple[int, float]) -> tuple[int, list[list[tuple[float, float]]]]:
            li, level = li_level
        ...
        with ThreadPoolExecutor(max_workers=num_workers) as executor:
            ...
            for li, polys in results:
                polylines_by_level[li] = polys
    else:
