...
def rotate_keep_size(
    ...
    Использует PIL с BILINEAR интерполяцией (быстрее BICUBIC при сопоставимом качестве).
    """
    spinner = LiveSpinner('Поворот карты')
    spinner.start()
    try:
        w, h = img.size
        # BILINEAR быстрее BICUBIC, качество достаточное для карт
        rotated = img.rotate(
            angle=angle_deg,
        ...
        # Центр-кроп к исходному размеру
        rw, rh = rotated.size
        cx, cy = rw // 2, rh // 2
        left = int(cx - w / 2)
        top = int(cy - h / 2)
