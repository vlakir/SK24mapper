...
async def process_elevation_contours(ctx: MapDownloadContext) -> Image.Image:
    ...
    logger.info(
        ...
        seed_w, seed_h, filled, seed_w * seed_h,
    )
    ...
    sp.start()
    try:
        result = Image.new('RGB', (ctx.crop_rect[2], ctx.crop_rect[3]), color=(255, 255, 255))
        ...
        if CONTOUR_LABELS_ENABLED:
            result = _add_contour_labels(
                result, seed_polylines, levels, ctx.crop_rect, seed_ds, ctx.center_lat_wgs, ctx.zoom
            )
    finally:
    ...
    *,
    ...
    try:
        logger.info(
            ...
            zoom, crop_rect[2], crop_rect[3], crop_rect[0], crop_rect[1], seed_ds, len(levels),
        )
        ...
        logger.info(
            ...
            mpp, TILE_SIZE, elev_retina_factor,
        )
        
        # Dry run to get label positions
        label_bboxes = draw_contour_labels(
            result, seed_polylines, levels, crop_rect, seed_ds, mpp, dry_run=True
        )
        ...
        if not label_bboxes:
            logger.warning(
                ...
                int(CONTOUR_LABEL_SPACING_M), int(CONTOUR_LABEL_MIN_SEG_LEN_M), int(CONTOUR_LABEL_EDGE_MARGIN_M),
            )
        ...
        if CONTOUR_LABEL_GAP_ENABLED and label_bboxes and not is_overlay:
            draw = ImageDraw.Draw(result)
            gap_padding = max(1, round(CONTOUR_LABEL_GAP_PADDING_M / max(1e-9, mpp)))
        ...
        placed_after = draw_contour_labels(
            result, seed_polylines, levels, crop_rect, seed_ds, mpp, dry_run=False
        )
...
async def apply_contours_to_image(
    ...
    overlay_retina_factor = max(1, int(round(ctx.full_eff_tile_px / 256)))
    if overlay_retina_factor > 2:
        overlay_retina_factor = 2
    ...
    logger.info(
        ...
        ctx.full_eff_tile_px, elev_tile_px, scale_factor,
        orig_cx, orig_cy, orig_cw, orig_ch,
        elev_cx, elev_cy, elev_cw, elev_ch,
    )
    ...
    logger.info(
        ...
        seed_w, seed_h, filled_count, seed_w * seed_h,
    ...
    try:
        ...
        logger.info(
            ...
            len(levels), total_polys, base_image.width, base_image.height,
        )
        ...
        effective_seed_ds = int(round(coord_scale))
        ...
        if CONTOUR_LABELS_ENABLED:
            ...
            label_bboxes = draw_contour_labels(
                result, seed_polylines, levels, (0, 0, img_w, img_h), effective_seed_ds, mpp, dry_run=True
            )
            logger.info('Overlay: получено %d позиций подписей для создания разрывов', len(label_bboxes))
        
        # Функция проверки пересечения сегмента с bbox (с учётом padding)
        gap_padding = 3  # пикселей отступа вокруг подписи
        ...
        for li, _level in enumerate(levels):
            ...
            for poly in seed_polylines.get(li, []):
                ...
                pts_img = [
                    (p[0] * coord_scale, p[1] * coord_scale)
                    for p in poly
                ]
        ...
        if CONTOUR_LABELS_ENABLED:
            result = _add_contour_labels(
                result, seed_polylines, levels, (0, 0, img_w, img_h), effective_seed_ds, ctx.center_lat_wgs, ctx.zoom,
                is_overlay=True,
