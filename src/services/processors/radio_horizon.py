...
from geo.topography import async_fetch_xyz_tile, assemble_dem, latlng_to_pixel_xy, meters_per_pixel
...
async def process_radio_horizon(ctx: MapDownloadContext) -> Image.Image:
    ...
    provider = ElevationTileProvider(
        client=ctx.client, api_key=ctx.api_key, use_retina=RADIO_HORIZON_USE_RETINA
    )
    ...
    if ds_factor > 1:
        logger.info(
            ...
            dem_w_orig, dem_h_orig, dem_w_orig * dem_h_orig // 1_000_000, ds_factor,
        )
    ...
    control_lng_wgs, control_lat_wgs = ctx.t_sk42_to_wgs.transform(cp_lng_sk42, cp_lat_sk42)
    ...
    pixel_size_m = meters_per_pixel(ctx.center_lat_wgs, ctx.zoom, scale=ctx.eff_scale) * ds_factor
