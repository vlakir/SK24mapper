...
# Жёсткое ограничение на число пикселей (автоматически понижает zoom)
# После оптимизации потоковой обработки RAM-потребление минимально (~100 MB),
# основное ограничение — место на диске для временного mmap-файла:
# 10B пикселей ≈ 30 GB mmap файл
MAX_OUTPUT_PIXELS = 10_000_000_000

# Мягкое ограничение на число пикселей для рисования оверлеев (сетка, легенда)
# После оптимизации потоковой обработки оверлеи рисуются полосами (~10 MB RAM)
MAX_OVERLAY_PIXELS = 10_000_000_000

# Мягкое ограничение для сохранения в memory-based форматах (JPEG, сжатый TIFF)
# При превышении выводится предупреждение
MAX_SAVE_PIXELS = 10_000_000_000
...
# --- In-memory кэш DEM тайлов (используется в geo/topography.py)
...
# --- Потоковая обработка больших изображений (streaming)
# Размер полосы (strip) для потоковой записи (пиксели по высоте)
STREAMING_STRIP_HEIGHT = 512
# Временная директория для mmap-файлов
STREAMING_TEMP_DIR = '.cache/streaming'
# Качество JPEG-сжатия в TIFF
TIFF_JPEG_QUALITY = 90
# Размер тайла в выходном TIFF
TIFF_TILE_SIZE = 512
# Использовать BigTIFF (для файлов >4GB)
TIFF_USE_BIGTIFF = True
# Удалять временные файлы после завершения
STREAMING_CLEANUP_TEMP = True
# Максимальный размер стороны JPEG (ограничение формата)
JPEG_MAX_DIMENSION = 65535
# Порог для полного пропуска поворота (в градусах)
# Только для углов, практически равных нулю
ROTATE_ANGLE_EPSILON = 0.001

# Порог для пропуска поворота ИЗОБРАЖЕНИЯ с переносом поворота на СЕТКУ (в градусах)
# Для углов < этого значения изображение НЕ поворачивается, вместо этого
# сетка рисуется под углом. Это экономит ~10 секунд на больших картах.
# Визуально разница незаметна для углов до 3°.
ROTATE_GRID_INSTEAD_THRESHOLD = 3.0

# Количество потоков для параллельного поворота (когда поворот всё же нужен)
ROTATE_NUM_WORKERS = 4

# Высота полосы для поворота (больше = меньше накладных расходов)
ROTATE_STRIP_HEIGHT = 1024

# Количество потоков для параллельного поворота
ROTATE_NUM_WORKERS = 4
...
# Максимальный размер базового уровня пирамиды (пиксели по большей стороне)
# Увеличено с 2048 до 8192 для лучшего качества превью при зуме
PYRAMID_BASE_MAX_SIZE = 8192
# Минимальный размер уровня пирамиды (пиксели по большей стороне)
PYRAMID_MIN_LEVEL_SIZE = 256
# Коэффициент уменьшения между уровнями пирамиды
PYRAMID_SCALE_FACTOR = 2
# Порог размера изображения для создания пирамиды (пиксели)
# Если изображение меньше этого порога, пирамида не создаётся
PYRAMID_THRESHOLD_PIXELS = 4_000_000  # ~2000x2000

# --- Тайловый кэш (SQLite)
# Директория кэша тайлов (абсолютный путь в домашней директории)
import os as _os
TILE_CACHE_DIR = _os.path.join(_os.path.expanduser('~'), '.sk24mapper', 'cache', 'tiles')
...
# Параллельность скачивания тайлов
TILE_FETCH_CONCURRENCY = 40

# Размер очереди записи в кэш
TILE_WRITE_QUEUE_SIZE = 1000

# Порог для автовыбора zoom (MB) — zoom выбирается чтобы файл был меньше
AUTO_ZOOM_TARGET_SIZE_MB = 500

# LRU буфер декодированных тайлов для превью
TILE_PREVIEW_LRU_SIZE = 100

# Источники тайлов
TILE_SOURCE_SATELLITE = 'satellite'
TILE_SOURCE_TERRAIN_RGB = 'terrain-rgb'
TILE_SOURCE_STREETS = 'streets'
TILE_SOURCE_OUTDOORS = 'outdoors'
