...
CENTER_LAT_SK42 = 48.87667        # Широта центра области в СК‑42 (градусы) — Золотарівка, Кременский район
CENTER_LNG_SK42 = 38.26917        # Долгота центра области в СК‑42 (градусы) — Золотарівка, Кременский район
WIDTH_M = 6000                    # Ширина требуемой области (метры)
HEIGHT_M = 4000                   # Высота требуемой области (метры)
...
async def download_satellite_rectangle(
    ...
    """
    # Подготовка — спиннеры по шагам
    sp = LiveSpinner("Подготовка: трансформеры")
    sp.start()
    t_sk42_to_wgs, t_wgs_to_sk42, crs_sk42_gk = build_transformers_sk42(center_lng_sk42)
    sp.stop("Подготовка: трансформеры готовы")

    sp = LiveSpinner("Подготовка: конвертация центра")
    sp.start()
    center_lng_wgs, center_lat_wgs = t_sk42_to_wgs.transform(center_lng_sk42, center_lat_sk42)
    sp.stop("Подготовка: центр готов")
